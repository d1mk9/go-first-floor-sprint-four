...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	// ваш код здесь
	a := runningCaloriesMeanSpeedMultiplier * meanSpeed(action, duration) * runningCaloriesMeanSpeedShift
	b := weight / mInKm * duration * minInH
	running := a * b
	return running
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	b := math.Pow(speed, 2) / height * cmInM
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	swimming := (speed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimming
}
